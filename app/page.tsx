...
import { TimelineProvider, useTimelineStore } from '../hooks/TimelineContext';
...
function DashboardAppContent() {
  const { activeTimelineId, timelines, stats, currentUser, enableTransitions, currentTab, setCurrentTab } = useTimelineStore();
  ...
  if (!currentUser) {
    ...
    return (
      <LandingPage 
        onEnterApp={() => setAuthMode('login')}
        onEnterAuth={(mode) => setAuthMode(mode || 'signup')}
      />
    );
  }
  ...
  return (
    <div className="w-full h-screen flex bg-slate-50 overflow-hidden font-sans select-none relative">
      ...
      <div className={`fixed inset-y-0 left-0 z-45 md:relative md:translate-x-0 ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-out shrink-0`}>
        <Sidebar currentTab={currentTab} setCurrentTab={(tab) => { setCurrentTab(tab); setMobileSidebarOpen(false); }} />
      </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main className="flex-1 overflow-hidden flex flex-col">
          <AnimatePresence mode="wait">
            <motion.div
              key={activeTimelineId ? `detail-${activeTimelineId}` : currentTab}
        ...
        <div className="bg-white border border-[#eaecf0] rounded-2xl p-6 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <img
              ...
              alt={currentUser?.username || "Totok Michael"}
              className={`w-12 h-12 rounded-full object-cover ring-4 ${activeTheme.avatarRing}`}
              referrerPolicy="no-referrer"
            />
            <div className="flex flex-col">
              <h4 className="font-bold text-slate-800 text-sm leading-tight">{currentUser?.username || "Totok Michael"}</h4>
              <p className="text-xs text-slate-500 mt-0.5">{currentUser?.email || "tmichael20@gmail.com"}</p>
            </div>
          </div>
        </div>
